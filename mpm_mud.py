...
class Example:
    def __init__(
        ...
        stage = Usd.Stage.Open(str((Path(__file__).parent /"scene_mud.usda").resolve()))
        # particle_offset = wp.vec3(0.5, 2, 0.5)
        # world_offset = wp.vec3(0.0, 4.0, 0.35)
        world_offset = wp.vec3(0.0, 0.0, 0.0)
        particle_offset = wp.vec3(0.0, 0.0, 0.12)
        global_scale = 1.0
        # parse particles
        # max_fraction = 1.0
        ...
        particles = parse_usd_particles(builder, "mud", stage, global_scale=1.0, global_translate=world_offset + particle_offset)
        # breakpoint()
        ...
        yield_pressure = np.array(particles["mpm_material"]["particle_material_yieldPressure"], dtype=float)
        yield_stress = np.array(particles["mpm_material"]["particle_material_yieldStress"], dtype=float)
        tensile_yield_ratio = np.array(particles["mpm_material"]["particle_material_tensileYieldRatio"], dtype=float)
        friction_array = np.array(particles["mpm_material"]["particle_material_friction"], dtype=float)
        hardening = np.array(particles["mpm_material"]["particle_material_hardening"], dtype=float)
        color = np.full((particles["pt_count"],3), [0.4, 0.25, 0.25])

        yp = yield_pressure
        ys = yield_stress
        tyr = tensile_yield_ratio
        frictions = friction_array
        hardening = hardening
        colors = color
        # parse scene 
        ...
        prim = stage.GetPrimAtPath("/World/Locomotion_Terrain_Small_collider_mesh/Mesh")
        ...
        # __output_collider_mesh_to_json(collision_wp_mesh, "C:/Users/legen/Desktop/corl_demo_final/Newton-Lightwheel/test/collider_mesh.json")
